...
URL_API_BASE = os.getenv("API_BASE_URL", "http://localhost:8080/api").rstrip("/")
...
def descargar_datos():
    ...
    for intento in range(1, REINTENTOS + 1):
        try:
            with requests.get(URL_DATOS, stream=True, timeout=TIMEOUT) as respuesta:
                respuesta.raise_for_status()
                with open(RUTA_DATOS, "wb") as archivo:
                    for bloque in respuesta.iter_content(chunk_size=1024 * 1024):
                        if bloque:
                            archivo.write(bloque)
            print("Datos geográficos descargados correctamente.")
            return
            ...
            if intento < REINTENTOS:
                espera = intento * 3
                print(f"Error descargando datos. Reintento {intento}/{REINTENTOS} en {espera}s - {error}")
                time.sleep(espera)
    raise ultimo_error
