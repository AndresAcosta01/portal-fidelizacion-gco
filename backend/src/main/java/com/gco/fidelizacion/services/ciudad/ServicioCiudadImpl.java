...
public class ServicioCiudadImpl implements IServicioCiudad {
    ...
    public CiudadResponseDTO crear(CiudadRequestDTO dto) {
        ...
        Departamento departamento = repositorioDepartamento.findById(dto.idDepartamento())
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "No se encontró el departamento"));
    ...
    public List<CiudadResponseDTO> listar() {
        return repositorioCiudad.findAll().stream().map(CiudadResponseDTO::fromEntity).toList();
    }

    @Override
    public List<CiudadResponseDTO> listarPorDepartamento(UUID idDepartamento) {
        return repositorioCiudad.findByDepartamento_IdAndActivoTrue(idDepartamento).stream()
    ...
    public CiudadResponseDTO buscarPorId(UUID idCiudad) {
        Ciudad ciudad = repositorioCiudad.findById(idCiudad)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "No se encontró la ciudad"));
